...
main() {
  ...
  if ! command -v sha256sum >/dev/null; then
    >&2 echo "$SCRIPT_NAME: Error: sha256sum command not found"
    exit 1
  fi
  ...
  for file in "${FILES[@]}"; do
    if [[ ! -f "$file" ]]; then
      >&2 echo "$SCRIPT_NAME: Error: File not found: $file"
      missing=1
    fi
  done
  ...
  # Generate new checksums file
  {
    ...
    echo "# Generated: $(TZ=UTC0 printf '%(%T)T UTC')"
  ...
  echo "$SCRIPT_NAME: Updated $CHECKSUM_FILE"
  echo "$SCRIPT_NAME: Files included:"
  for file in "${FILES[@]}"; do
    echo "  - $file"
  done
}
